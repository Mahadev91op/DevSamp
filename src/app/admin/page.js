...
import { 
  ...
  ChevronLeft, ChevronRight, Image as ImageIcon, Maximize2, Minimize2, BarChart3, Activity, ArrowRight, Zap
} from "lucide-react";
...
// --- GRADIENT OPTIONS FOR PRICING (Defined ONCE here) ---
            ...
            <div className="flex bg-black border border-white/10 rounded-lg p-1 overflow-x-auto max-w-full">
                {['1D', '7D', '1M', '3M', '1Y'].map(r => (
                    <button 
                        ...
                        className={`px-3 py-1 text-[10px] font-bold rounded-md transition-all whitespace-nowrap ${timeRange === r ? `bg-${color}-600 text-white` : "text-gray-500 hover:text-white"}`}
                    >
                        {r}
                    </button>
                ))}
            </div>
            <button onClick={onToggleExpand} className="p-2 bg-white/5 hover:bg-white/10 rounded-lg text-gray-400 hover:text-white transition-colors">
        ...
        <AnimatePresence>
            {hoveredVal !== null && (
                <motion.div 
                    ...
                    className={`absolute bg-${color}-600 text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-xl pointer-events-none transform -translate-x-1/2 z-20`}
                    style={{ left: `${(hoveredIndex / (data.length - 1)) * 100}%`, top: points[hoveredIndex][1] - 40 }}
                >
                    {hoveredVal} Leads
                    <div className={`absolute bottom-[-4px] left-1/2 -translate-x-1/2 w-2 h-2 bg-${color}-600 rotate-45`}></div>
                </motion.div>
            )}
        </AnimatePresence>
        ...
        <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-full overflow-visible preserve-3d">
            <defs>
                <linearGradient id={`grad-${color}`} x1="0" x2="0" y1="0" y2="1">
                    <stop offset="0%" stopColor={color === 'blue' ? '#3b82f6' : '#a855f7'} stopOpacity="0.4" />
                    <stop offset="100%" stopColor={color === 'blue' ? '#3b82f6' : '#a855f7'} stopOpacity="0" />
                </linearGradient>
            </defs>
            <line x1="0" y1={h} x2={w} y2={h} stroke="#222" strokeWidth="1" />
            <line x1="0" y1={0} x2={w} y2={0} stroke="#222" strokeWidth="1" strokeDasharray="4 4" />
            ...
            <circle 
                    key={i} cx={x} cy={y} r="6" 
                    fill="#0a0a0a" stroke={color === 'blue' ? '#3b82f6' : '#a855f7'} strokeWidth="3"
                    className="cursor-pointer transition-all duration-200 hover:r-8 hover:fill-white hover:stroke-white z-10"
                    onMouseEnter={() => { setHoveredVal(data[i]); setHoveredIndex(i); }}
                    onMouseLeave={() => { setHoveredVal(null); setHoveredIndex(null); }}
            />
            ))}
        </svg>

        <div className="flex justify-between mt-4 text-[10px] text-gray-500 font-mono font-bold uppercase tracking-widest">
            {labels.map((l, i) => (
                <span key={i} className={`${i % 2 !== 0 && !expanded ? "hidden" : "block"}`}>{l}</span>
            ))}
        </div>
      </div>
    </div>
    ...
    return (
        <div className={`bg-[#0a0a0a] border border-white/5 p-6 rounded-3xl transition-all duration-500 ease-in-out relative overflow-hidden ${expanded ? "col-span-full row-span-2 z-50 scale-[1.01] shadow-2xl" : "col-span-1 shadow-sm"}`}>
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h3 className={`font-bold text-white flex items-center gap-2 ${expanded ? "text-2xl" : "text-lg"}`}>
                        <BarChart3 size={expanded ? 24 : 18} className={`text-${color}-500`}/> Popular Services
                    </h3>
                    <p className="text-gray-500 text-xs mt-1">Most requested services by clients.</p>
                </div>
                <button onClick={onToggleExpand} className="p-2 bg-white/5 hover:bg-white/10 rounded-lg text-gray-400 hover:text-white transition-colors">
                    {expanded ? <Minimize2 size={16}/> : <Maximize2 size={16}/>}
                </button>
            ...
            <div className={`w-full flex items-end justify-between gap-2 md:gap-4 ${expanded ? "h-[400px]" : "h-[200px]"}`}>
                {data.map((val, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center justify-end h-full group relative">
                        <div className="absolute -top-10 opacity-0 group-hover:opacity-100 transition-opacity bg-white text-black text-xs font-bold px-2 py-1 rounded mb-2 z-10 pointer-events-none">
                            {val}
                        </div>
                        <motion.div 
                            initial={{ height: 0 }}
                            ...
                            className={`w-full max-w-[40px] rounded-t-lg bg-gradient-to-t from-${color}-900/50 to-${color}-500/80 border-t border-x border-${color}-400/30 hover:to-${color}-400 transition-all cursor-pointer relative overflow-hidden`}
                        >
...
const Toast = ({ message, type, onClose }) => (
  <motion.div 
    ...
    className={`fixed top-6 left-1/2 -translate-x-1/2 z-[200] flex items-center gap-3 px-6 py-3 rounded-full shadow-2xl backdrop-blur-xl border ${
...
const StatCard = ({ title, value, icon: Icon, color, trend, onClick }) => (
  <div 
    onClick={onClick}
    className="bg-[#0f0f0f] border border-white/5 p-6 rounded-2xl relative overflow-hidden group hover:border-white/10 transition-all hover:-translate-y-1 duration-300 shadow-xl cursor-pointer"
  >
    <div className={`absolute -right-6 -top-6 w-24 h-24 bg-${color}-500/10 blur-2xl rounded-full group-hover:bg-${color}-500/20 transition-all`}></div>
    ...
    <div className="flex items-center gap-2">
        ...
        <ArrowRight size={12} className="text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity -translate-x-2 group-hover:translate-x-0" />
    </div>
  </div>
);
...
const Pagination = ({ total, perPage, current, onChange }) => {
    ...
    return (
        <div className="flex items-center justify-end gap-2 mt-4">
            <button disabled={current === 1} onClick={() => onChange(current - 1)} className="p-2 rounded-lg border border-white/10 hover:bg-white/5 disabled:opacity-50 disabled:cursor-not-allowed"><ChevronLeft size={16}/></button>
            <span className="text-xs text-gray-400">Page {current} of {pages}</span>
            <button disabled={current === pages} onClick={() => onChange(current + 1)} className="p-2 rounded-lg border border-white/10 hover:bg-white/5 disabled:opacity-50 disabled:cursor-not-allowed"><ChevronRight size={16}/></button>
        </div>
    );
};
...
export default function AdminPanel() {
  ...
  const lineChartData = useMemo(() => {
    return filterAndGroupData(data.leads, leadsTimeRange);
  }, [data.leads, leadsTimeRange]);

  const barChartData = useMemo(() => {
    const counts = {};
    data.leads.forEach(l => { const svc = l.service || "General"; counts[svc] = (counts[svc] || 0) + 1; });
  ...
  const getTypeFromTab = (tab) => {
    const map = { projects: 'project', services: 'service', blogs: 'blog', team: 'team', pricing: 'pricing', reviews: 'review' };
    return map[tab] || null;
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const apiMap = { project: 'projects', service: 'services', blog: 'blogs', team: 'team', pricing: 'pricing', review: 'reviews' };
        ...
        <aside className={`fixed md:relative z-50 w-72 h-full bg-[#0a0a0a] border-r border-white/5 flex flex-col transition-transform duration-300 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
            <div className="p-6 border-b border-white/5 flex justify-between items-center">
                <div className="flex items-center gap-3 font-bold text-xl tracking-tight">
                    <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-xs shadow-blue-500/50 shadow-md">DS</div>
                    <span>DEVSAMP<span className="text-blue-500 text-xs align-top ml-1">ADMIN</span></span>
                </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
                ...
                <NavItem icon={ExternalLink} label="Projects" id="projects" active={activeTab} set={(id) => { setActiveTab(id); setSidebarOpen(false); }} />
        ...
        <main className="flex-1 flex flex-col h-full overflow-hidden relative">
            <header className="h-16 border-b border-white/5 flex items-center justify-between px-4 md:px-6 bg-[#050505]/80 backdrop-blur-xl z-20">
                <div className="flex items-center gap-3 md:gap-4">
                    <button onClick={() => setSidebarOpen(true)} className="md:hidden text-gray-400 p-1"><Menu size={24}/></button>
                    <h2 className="text-base md:text-lg font-bold capitalize flex items-center gap-2 truncate">
                        {activeTab} <span className="text-gray-600 font-normal text-sm hidden sm:inline">/ Management</span>
                    </h2>
                </div>
                <div className="flex items-center gap-3 md:gap-4">
                    <button onClick={fetchAllData} className={`p-2 rounded-full hover:bg-white/10 text-gray-400 transition-all ${loading && "animate-spin text-blue-500"}`} title="Refresh Data"><Loader2 size={18}/></button>
                    <div className="h-8 w-8 rounded-full bg-gradient-to-br from-gray-700 to-gray-900 border border-white/20 flex items-center justify-center text-xs font-bold">A</div>
                </div>
            </header>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar relative">
                ...
                {activeTab === 'dashboard' && (
                    <div className="space-y-6 md:space-y-8 animate-fade-in max-w-7xl mx-auto pb-20 md:pb-0">
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <StatCard title="Live Projects" value={data.projects.length} icon={Briefcase} color="purple" onClick={() => setActiveTab('projects')} />
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <LineChart 
                                ...
                                onToggleExpand={() => setExpandedChart(expandedChart === 'line' ? null : 'line')}
                                timeRange={leadsTimeRange}
                                setTimeRange={setLeadsTimeRange}
                                color="blue"
                            />
                            <BarChart 
                                ...
                                onToggleExpand={() => setExpandedChart(expandedChart === 'bar' ? null : 'bar')}
                                color="purple"
                            />
                        </div>
                        ...
                        <div className="bg-[#0a0a0a] border border-white/5 p-6 rounded-2xl flex flex-col">
                            <h3 className="font-bold text-lg mb-4 flex items-center gap-2"><CheckCircle size={18} className="text-green-500"/> Recent Inquiries</h3>
                            <div className="space-y-3 flex-1 overflow-y-auto custom-scrollbar pr-2 max-h-[300px]">
                                {data.leads.slice(0, 5).map(l => (
                                    ...
                                    >
                                        <div className="w-10 h-10 rounded-full bg-blue-500/20 text-blue-400 flex items-center justify-center text-sm font-bold shrink-0">{l.name.charAt(0)}</div>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm font-medium truncate text-white">{l.name}</p>
                                            <p className="text-xs text-gray-500 mt-0.5 truncate">{l.service} • {new Date(l.createdAt).toLocaleDateString()}</p>
                                        </div>
                                        <div className="flex flex-col items-end gap-1">
                                            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${l.status === 'New' ? 'bg-blue-500/20 text-blue-400' : l.status === 'Closed' ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'}`}>
                                                {l.status}
                                            </span>
                                            <ArrowRight size={14} className="text-gray-600 group-hover:text-white transition-colors opacity-0 group-hover:opacity-100" />
                                        </div>
                                    </div>
                                ))}
                                {data.leads.length === 0 && <p className="text-sm text-gray-500 italic">No recent activity.</p>}
                            </div>
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'leads' && (
                    <div className="space-y-4 max-w-7xl mx-auto animate-fade-in pb-20 md:pb-0">
                        <div className="flex flex-col md:flex-row gap-4 justify-between bg-[#0a0a0a] border border-white/5 p-4 rounded-2xl sticky top-0 z-30 backdrop-blur-md bg-opacity-90">
                            <div className="relative w-full md:w-96 group">
                                <Search className="absolute left-3 top-3 text-gray-500 group-focus-within:text-blue-500 transition-colors" size={18} />
                            ...
                            <div className="flex gap-2">
                                <button onClick={fetchAllData} className="p-2.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-gray-300 transition-colors"><Loader2 size={18} className={loading ? "animate-spin" : ""}/></button>
                            </div>
                        </div>
                        ...
                        <div className="bg-[#0a0a0a] border border-white/5 rounded-2xl overflow-hidden shadow-xl">
                            <div className="overflow-x-auto">
                                <table className="w-full text-left border-collapse min-w-[800px] md:min-w-0">
                                    <thead>
                                        <tr className="bg-white/[0.02] text-gray-400 text-xs uppercase tracking-wider border-b border-white/5">
                                            ...
                                            <th className="p-5 font-medium">Interest</th>
                                        ...
                                        {paginatedData.map((lead) => (
                                            <tr key={lead._id} className="hover:bg-white/[0.02] transition-colors group">
                                                <td className="p-5">
                                                    ...
                                                    <div className="text-xs text-gray-500 flex items-center gap-1 mt-1"><Mail size={10}/> {lead.email}</div>
                                                </td>
                                                <td className="p-5">
                                                    <span className="bg-blue-500/10 text-blue-400 border border-blue-500/20 px-2.5 py-1 rounded-lg text-xs font-medium">{lead.service}</span>
                                                </td>
                                                ...
                                                <td className="p-5">
                                                    <select 
                                                        ...
                                                        className={`appearance-none pl-3 pr-8 py-1.5 rounded-lg text-xs font-bold outline-none cursor-pointer border transition-all ${
                                                ...
                                                <td className="p-5 text-right">
                                                    <div className="flex justify-end gap-2 opacity-100 md:opacity-60 md:group-hover:opacity-100 transition-opacity">
                                                        <button onClick={() => setViewLead(lead)} className="p-2 bg-white/5 hover:bg-white/10 rounded-lg text-blue-400 transition-colors" title="View Details"><Eye size={16}/></button>
                                                        <a href={`mailto:${lead.email}?subject=Re: Your Inquiry for ${lead.service}&body=Hi ${lead.name},%0D%0A%0D%0AThank you for contacting DevSamp regarding ${lead.service}.`} className="p-2 bg-white/5 hover:bg-white/10 rounded-lg text-green-400 transition-colors" title="Reply"><Mail size={16}/></a>
                                                        <button onClick={() => handleDelete('contact', lead._id)} className="p-2 bg-white/5 hover:bg-red-500/20 text-red-500 rounded-lg transition-colors" title="Delete"><Trash2 size={16}/></button>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
                                ...
                                {filteredData.length === 0 && <div className="p-10 text-center text-gray-500 bg-[#0a0a0a]">No leads found matching your search.</div>}
                            </div>
                ...
                <div className="fixed inset-0 z-[150] flex items-center justify-center px-4 bg-black/80 backdrop-blur-sm p-4">
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-[#111] border border-white/10 p-8 rounded-3xl w-full max-w-lg shadow-2xl relative overflow-hidden max-h-[90vh] overflow-y-auto custom-scrollbar">
                        ...
                        <div className="flex justify-between items-center mb-6 relative z-10">
                            <h3 className="text-2xl font-bold text-white capitalize">{forms[modalType].id ? "Edit" : "Add New"} {modalType}</h3>
                            ...
                            {modalType === 'pricing' && (
                                <>
                                    <div className="flex items-center justify-between bg-white/5 p-3 rounded-xl border border-white/10 mb-2">
                                        ...
                                        <button 
                                            type="button"
                                            onClick={() => setForms(p => ({...p, pricing: {...p.pricing, popular: !p.pricing.popular}}))}
                                            className={`w-12 h-6 rounded-full p-1 transition-colors ${forms.pricing.popular ? 'bg-blue-600' : 'bg-gray-700'}`}
                                        >
                                    ...
                                    <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
                                        {gradientOptions.map((g) => (
                                            <button
                                                key={g.name}
                                                type="button"
                                                onClick={() => setForms(p => ({...p, pricing: {...p.pricing, gradient: g.class}}))}
                                                className={`w-8 h-8 rounded-full bg-gradient-to-br ${g.class} ring-2 ring-offset-2 ring-offset-black transition-all ${forms.pricing.gradient === g.class ? 'ring-white scale-110' : 'ring-transparent opacity-70 hover:opacity-100'}`}
                                                title={g.name}
                                            />
                                        ))}
                                    </div>
    ...
    <button onClick={() => set(id)} className={`flex items-center gap-3 w-full px-4 py-3 rounded-xl transition-all text-sm font-medium group ${active === id ? "bg-white text-black font-bold shadow-lg shadow-white/5" : "text-gray-400 hover:text-white hover:bg-white/5"}`}>
        ...
        {badge > 0 && (
            <span className="ml-auto flex items-center justify-center">
                <span className="animate-ping absolute inline-flex h-2 w-2 rounded-full bg-blue-400 opacity-75 mr-4"></span>
                <span className="bg-blue-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-md shadow-blue-600/20 relative z-10">{badge}</span>
            </span>
        )}
    </button>
);
...
const FormInput = ({ label, value, onChange, preview, placeholder }) => (
    <div className="space-y-1 w-full">
        ...
        <div className="flex gap-4">
            ...
            {preview && value && (
                <div className="w-12 h-11 relative rounded-lg overflow-hidden border border-white/20 shrink-0 bg-gray-800">
